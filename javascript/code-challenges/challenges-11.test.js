...
const totalSum = (input) => {
  ...
  return input.reduce((total, subArray) => {
    const subTotal = subArray.reduce((acc, num) => acc + num, 0);
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  return input.map(subArray => {
    ...
    return filteredArray.map(num => Math.pow(2, num));
  });
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  const filteredNames = data.filter(character => character.gender === 'male' || character.gender === 'female').map(character => character.name).join(' and ');
...
let findShortest = (data) => {
  // Solution code here...
  const validData = data.filter(character => !isNaN(character.height));

  const shortestCharacter = validData.reduce((shortest, current) => {
    return parseInt(current.height) < parseInt(shortest.height) ? current : shortest;
  });
